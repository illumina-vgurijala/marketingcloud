...
(function($) {
  ...
  $.extend($.flexselect.prototype, {
    ...
    filterResults: function() {
	  ...
	  var startsWith = function(source, searchString, position) {
		position = position || 0;
		if(!source)source="";
		return source.toLowerCase().indexOf(searchString.toLowerCase(), position) === position;
	  };
  
      var results = [];
      $.each(this.cache, function() {
        if (this.disabled && !showDisabled) return;
        this.score = 1.0;//LiquidMetal.score(this.text, abbreviation);
        //if (this.score > 0.0) 
		if(startsWith(this.text,abbreviation))
			results.push(this);
      });
