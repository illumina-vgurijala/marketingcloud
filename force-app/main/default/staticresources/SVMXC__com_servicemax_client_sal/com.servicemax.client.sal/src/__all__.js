// Code: C:\var\lib\jenkins\workspace\ReleaseOps\SVMX_UIFW_SUM20\src\modules\com.servicemax.client.sal\src\impl.js
/**
...
;(function () {
    ...
    salServiceImpl.Class("RuntimeMode", com.servicemax.client.lib.api.Object, {
        appInstance: "",
        url: "/services/data/",
        apexRestUrl: "/services/apexrest/",
        soapUrl: "/services/Soap/u/",
        apexSoapUrl: "/services/Soap/class/",
        __constructor: function () {
        },
        init: function () {
        },
    ...
    salServiceImpl.Class("VFRuntimeMode", salServiceImpl.RuntimeMode, {
        ...
        adjustHeaders: function (headers, params, apiVersion) {
            var instanceName = SVMX.getClient().getApplicationParameter("instanceName");
            if(instanceName){
                headers["SalesforceProxy-Endpoint"] = "https://" + instanceName + ".salesforce.com" + this.url + apiVersion + "/" + params.url;
            } else {
                headers["SalesforceProxy-Endpoint"] = "https://" + this.appInstance + ".salesforce.com" + this.url + apiVersion + "/" + params.url;
            }
        },
        ...
        adjustApexRestHeaders: function (headers, params) {
            var instanceName = SVMX.getClient().getApplicationParameter("instanceName");
            if(instanceName){
                headers["SalesforceProxy-Endpoint"] = "https://" + instanceName + ".salesforce.com" + this.apexRestUrl + params.url;
            } else {
                headers["SalesforceProxy-Endpoint"] = "https://" + this.appInstance + ".salesforce.com" + this.apexRestUrl + params.url;
            }
        },
        ...
        getSoapUrl: function (url, apiVersion) {
            return this.soapUrl + apiVersion + "/" + url;
        },
        getApexSoapUrl: function (url) {
            return this.apexSoapUrl + url;
        }
    ...
    salServiceImpl.Class("RPRuntimeMode", salServiceImpl.RuntimeMode, {
        ...
        getUrl: function (url, apiVersion) {
            return this.url + apiVersion + "/" + url;
        },

        getApexRestUrl: function (url) {
            return this.apexRestUrl + url;
        },
    ...
    salServiceImpl.Class("SAL", com.servicemax.client.lib.api.Object, {}, {
        ...
        init: function () {
            ...
            if (salServiceImpl.SAL.initialized) return;

            salServiceImpl.SAL.initialized = true;
            ...
            callAsync: function (params) {
                ...
                return SVMX.ajax({type: method,
                    ...
                    error: function (jqXhr, status, e) {
                        this._callAsyncError(jqXhr, status, e);
                    }});
            },
    ...
    salServiceImpl.Class("AttachmentSoapRequest", com.servicemax.client.lib.api.EventDispatcher, {
        _options: null, _baseURL: "", _requestTemplate: "", _recordId: null,

        __constructor: function (options) {
            this.__base();
            this._options = options;

            this._requestTemplate = "<s:Envelope xmlns:s='http://schemas.xmlsoap.org/soap/envelope/' xmlns='urn:partner.soap.sforce.com'>"
                +"<s:Header>"
                +"<SessionHeader><sessionId>{0}</sessionId></SessionHeader>"
                +"</s:Header>"
                +"<s:Body>"
                +"<create><sobject>"
                +"<type>{1}</type>"
        ...
        callAsync: function (params) {
            ...
            var url = this._getUrl(params.url), method = "POST",
                ...
                this._recordId = params.questionId;
        ...
        _callAsyncAttachmentSuccess:function(data, status, jqXhr){
            var parser = new DOMParser();
            var xmlDoc = parser.parseFromString(jqXhr.responseText,"text/xml");
            var id = '';
            if(xmlDoc.getElementsByTagName("id") && xmlDoc.getElementsByTagName("id").length > 0){
                var idElement = xmlDoc.getElementsByTagName("id")[0];
                if(idElement.childNodes && idElement.childNodes.length >0){
                    id = idElement.childNodes[0].nodeValue;
                }
            }
            data = this._processDataOnLoad(data);
            data.attachmentId = id;
            data.questionId = this._recordId;
        ...
        _getData: function (data, params) {
            ...
            data = this._format(this._requestTemplate, this._options.sessionId,
                params.type,params.isPrivate,params.ParentId,params.name,
                params.body
            );
            return data;
        },
    ...
    salServiceImpl.Class("WSSoapRequest", salServiceImpl.SoapRequest, {

        __constructor: function (options) {
            this.__base(options);
            this._requestTemplate = '<se:Envelope xmlns:se="http://schemas.xmlsoap.org/soap/envelope/" xmlns:sfns="http://soap.sforce.com/schemas/class/{0}">' +
                    '<se:Header>' +
                        '<sfns:SessionHeader><sfns:sessionId>{1}</sfns:sessionId></sfns:SessionHeader>' +
                    '</se:Header>' +
                    '<se:Body><sfns:{2}>' +
                    '<sfns:request>{3}</sfns:request>' +
                    '</sfns:{2}></se:Body>' +
                '</se:Envelope>';
        },
        ...
        _getData: function (dataXML, params) {
            //data = SVMX.toXML(data); // Expected to already be XML string

            data = this._format(this._requestTemplate,
                this._options.endPoint,
                this._options.sessionId,
                params.methodName,
                dataXML
    ...
    salServiceImpl.Class("SoapService", salServiceImpl.ServiceBase, {
        ...
        callMethodAsync: function (params) {
            ...
            if(params.type === 'Attachment'){
                sr = new salServiceImpl.AttachmentSoapRequest({sessionId: this._parent.params.sessionId,
                ...
                sr = new salServiceImpl.ApexSoapRequest({sessionId: this._parent.params.sessionId,
                apiVersion: this.apiVersion, endPoint: endPoint, nameSpace: nameSpace});
                sr.bind("REQUEST_COMPLETED", this._callMethodAsyncSuccess, this);
                sr.bind("REQUEST_ERROR", this._callMethodAsyncError, this);
