	...
	__onDispatch(action) {
		...
		switch (action.type) {
			...
			case a.TEMPLATE_RECEIVED: {
				...
				data.template.hasLockedRegions = data.template.html.includes('pardot-region');
...
function getUnsubLinkHtml(calledOnTabSwitch) {
	...
	if (data.template.html.includes('pardot-region') && calledOnTabSwitch) {
		if (window.hasHmlEnabled) {
...
function getEmailParamsForSend() {
	...
	let {template, recipientType} = data;
