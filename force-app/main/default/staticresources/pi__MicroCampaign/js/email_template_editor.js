...
export default function EmailTemplateEditor(ckEditor, containerId, htmlConverter, jQuery) {
	...
	this.editor.on('key', function(event) {
		let body = document.querySelector(EmailTemplateConstants.ckeditorClass).contentWindow.document.body;
...
export function removeHighlightDivs() {
	...
	body = removeHighlightElementsFrom(body);
	$(EmailTemplateConstants.ckeditorClass).contents()[0].body.innerHTML = body;
}

function removeHighlightElementsFrom(body) {
	let unWrappedBody = $('<div />').append(body)
	//remove parent div (the highlighting)
		.find(`.${EmailTemplateConstants.unsupportedTagIdentifier}`).children().unwrap().end().end()
		//remove warning icon
		.find(`svg[id^="${EmailTemplateConstants.unsupportedTagSvgIdPrefix}"]`).each((index, node) => {
			node.parentNode.removeChild(node)
		}).end()
		//remove any empty spans (just in case)
		.find(`.${EmailTemplateConstants.unsupportedTagIdentifier}:empty`).each((index, node) => {
			node.parentNode.removeChild(node)
		}).end();
	...
	if (unWrappedBody.html().includes("pardot-region")) {
		unWrappedBody
			...
			.find(`.${EmailTemplateConstants.unsupportedTagBufferIdentifier}`).children().unwrap().end().end()
			//remove parent span - if there are only text children
			.find(`.${EmailTemplateConstants.unsupportedTagBufferIdentifier}`).each(function() {
				$(this).replaceWith(this.childNodes)
			}).end()
			// remove empty buffer span
			.find(`.${EmailTemplateConstants.unsupportedTagBufferIdentifier}`).each((index, node) => {
				node.parentNode.removeChild(node)
			}).end();
	}
...
export function insertAttachmentImage(imageFile, width, height) {
	...
	if (bookmarks.length > 0 && (!isFirefoxLockedTemplate || (isFirefoxLockedTemplate && isSelectionInUnlockedRegion))) {
		bookmarks[0].startNode.addClass(EmailTemplateConstants.imageAttachmentIdentifier);
		let imagePlaceholder = body.querySelector(`.${EmailTemplateConstants.imageAttachmentIdentifier}`);
...
export function insertHmlMergeField(mergeField) {
	...
	if (bookmarks.length > 0) {
		bookmarks[0].startNode.addClass(EmailTemplateConstants.hmlMergeFieldPlaceHolderIdentifier);
		let mergeFieldPlaceholder = body.querySelector(`.${EmailTemplateConstants.hmlMergeFieldPlaceHolderIdentifier}`);
...
export function addHighlightDivs(tags, body) {
	...
	if (!body) {
		body = $(EmailTemplateConstants.ckeditorClass).contents()[0].body.innerHTML;
	}
...
function getHighlightSpan(i, dedupedTags, svgPath, templateContainsLockedRegions) {
	let span =
		`<span contenteditable="false" class="${EmailTemplateConstants.unsupportedTagIdentifier}" title="Unsupported Variable Tag">` +
		`<svg id="${EmailTemplateConstants.unsupportedTagSvgIdPrefix + i}" width="1em" height="1em" viewBox="0 0 24 24">` +
		svgPath +
		'</svg>' +
		dedupedTags[i] +
		'</span>';
	...
	if (templateContainsLockedRegions) {
		span += `<span class="${EmailTemplateConstants.unsupportedTagBufferIdentifier}"></span>`;
	}
...
export function getEditorBodyText() {
	return $(EmailTemplateConstants.ckeditorClass).contents()[0].body.innerHTML;
}
...
EmailTemplateEditor.prototype.getTemplate = function ()
{
	...
	if (this.template) {
		...
		return {
			...
			type: EmailTemplateConstants.types.TEXT_HTML,
...
function stripCkEditorBsTitleElement() {
	let editorBody = document.querySelector(EmailTemplateConstants.ckeditorClass).contentWindow.document.body;
...
EmailTemplateEditor.prototype.setTemplateBodyContentEditable = function (contentEditable)
{
	...
	var doc = $(EmailTemplateConstants.ckeditorClass).contents()[0];
	if (doc !== undefined){
...
function unlockedRegionsTemplateDivAdd()
{
	...
	var hasAddedIeDivs = $(EmailTemplateConstants.ckeditorClass).contents()[0].querySelectorAll("[sfdc-internal-mc-ieSpecialAddedLockedTemplateDiv]").length > 0;
	if (hasAddedIeDivs) {
		return;
	}
	var nodeNames = ["TABLE", "COL", "COLGROUP", "TBODY", "TD", "TFOOT", "TH", "THEAD", "TR"];
	var pardotRegions = $(EmailTemplateConstants.ckeditorClass).contents()[0].querySelectorAll("[pardot-region]");
	...
	for (var i = 0; i < pardotRegions.length; i++) {
		...
		if (nodeNames.indexOf(region.nodeName) >= 0) {
			...
			var contentEditableDiv = $(EmailTemplateConstants.ckeditorClass).contents()[0].createElement('div');
...
{
	// setTemplate could be called more than once and add these twice, don't add again if already added
	var hasAddedImageDivs = $(EmailTemplateConstants.ckeditorClass).contents()[0].querySelectorAll("[sfdc-internal-mc-unlockImagesWithDiv]").length > 0;
	if (hasAddedImageDivs) {
		return;
	}
	var images = $(EmailTemplateConstants.ckeditorClass).contents()[0].querySelectorAll("img[pardot-region]");
...
{
	var hasAddedParagraphStyle = $(EmailTemplateConstants.ckeditorClass).contents()[0].getElementById('sfdc-internal-mc-sfEngagetemplateLockingAddedP');
	if (hasAddedParagraphStyle) {
		return;
	}
	var templateParagraphs = $(EmailTemplateConstants.ckeditorClass).contents()[0].getElementsByTagName("P");
	var templateBody = $(EmailTemplateConstants.ckeditorClass).contents()[0].head;
	...
	var style = $(EmailTemplateConstants.ckeditorClass).contents()[0].createElement('style');
	style.setAttribute('id', 'sfdc-internal-mc-sfEngagetemplateLockingAddedP');
	style.type = 'text/css';
	var css = 'p:not(.nonCkeditorAddedP){display: inline;}';
	style.appendChild($(EmailTemplateConstants.ckeditorClass).contents()[0].createTextNode(css));
...
EmailTemplateEditor.prototype.setTemplate = function(newTemplate)
{
	...
	if (this.template.html.includes("pardot-region")) {
	...
	let data = type == EmailTemplateConstants.types.TEXT_ONLY ? text : html;

	if (type == EmailTemplateConstants.types.TEXT_ONLY) {
		data = data.replace(/\r\n/g, '\n').replace(/\r/g, '\n');
		data = data.replace(/\n/g, '<br />');
	} else {
		// this fixes lack of cursor visibility when tabbed to W-4497228
		if (isFireFox() && this.template.id == '-1') {
			data = '&#8203' + data;
		}
	}
	...
	return new Promise((resolve, reject) => {
		...
		if (this.editor.getData().includes('pardot-region')) {
			var isIE = !!navigator.userAgent.match(/Trident/g) || !!navigator.userAgent.match(/MSIE/g) || !!navigator.userAgent.match(/Edge/g);
			if (isIE) {
