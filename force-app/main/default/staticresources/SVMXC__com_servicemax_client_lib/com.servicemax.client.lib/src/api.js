(function ($) {
  ...
  libApi.Class('EventDispatcher', libApi.Object, {
    ...
    unbind: function (type, handler, context) {
      for (var i = 0; i < this.eventHandlers.length; i++) {
        if (this.eventHandlers[i].handler === handler && this.eventHandlers[i].type === type && this.eventHandlers[i].context === context) {
    ...
    triggerEvent: function (e) {
      ...
      for (var i = 0; i < events.length; i++) {
        if (events[i].context) {
                    //bind arguments
          try {
            events[i].handler && events[i].handler.call(events[i].context, e);
          } catch (err) {
            console.error(err);
          } finally {
                        // do nothing
  ...
  libApi.Class('ExtensionRunner', com.servicemax.client.lib.api.Object, {
    ...
    perform: function (inParams) {
      ...
      if(inParams && inParams.extensionClassName && inParams.extensionClassName!=''){

        var extClass = SVMX.create(inParams.extensionClassName);
      ...
      }else{//Run All Extension as it has been existing
        ...
        // STATIC METHODS
    {
      ...
      run: function (inCaller, inName, inParams) {
        //If testing, skip extensions loading
        if(SVMX.TEST) {
          console.log('skipping extension runner');
          var d = SVMX.Deferred();
          d.resolve();
          return d;
        }
