(function ($) {
  ...
  libServices.Class('ProfilingService', com.servicemax.client.lib.api.EventDispatcher, {
    ...
    __getBrowser: function(){
      var userAgentLocal=navigator.userAgent,tem;
      var browserOptions = 'chrome|opera|firefox|safari|trident|msie';
      var versionRegex = '\\s*([0-9]+)';
      var regex = new RegExp('(' + browserOptions + ')' + '\/?' + versionRegex, 'i');
      var localBrowser = regex.exec(userAgentLocal) || [];
      ...
      var browser = '';
      var temForChrome = /\bOPR\/(\d+)/;
      if(localBrowser[1] === chromeBrowser){
        tem=userAgentLocal.match(temForChrome);
        if(tem!=null)   {
          browser = operaBrowser + ' ' +tem[1];
        }
      } else if(/trident/i.test(localBrowser[1])){
        tem=/\brv[ :]+(\d+)/g.exec(userAgentLocal) || [];
        browser = ieBrowser;
      }

      if(!browser) {
        var temForOthers = /version\/(\d+)/i;
        var browserDetails = [localBrowser[1], localBrowser[2]];
        var navigatorDetails = [navigator.appName, navigator.appVersion, '-?'];

        localBrowser=localBrowser[2]? browserDetails: navigatorDetails;

        if((tem=userAgentLocal.match(temForOthers))!=null) {
          localBrowser.splice(1,1,tem[1]);
        }
        browser = localBrowser[0];
      }
    ...
    __getBrowserVersion: function(){
      ...
      if(localBrowser[1] === chromeBrowser){
        tem=userAgentLocal.match(temForChrome);
        if(tem!=null)   {
          browserVersion = operaBrowser + ' ' + tem[1];
        }
      } else if(/trident/i.test(localBrowser[1])){
        tem=/\brv[ :]+(\d+)/g.exec(userAgentLocal) || [];
        browserVersion = tem[1] || '0';
      }
    ...
    getEntriesByName: function(name) {
      ...
      this.__performance.getEntriesByName && $.merge(results, this.__performance.getEntriesByName(name));
      return results;
    },
    ...
    getEntriesByType: function(type) {
      ...
      this.__performance.getEntriesByType && $.merge(results, this.__performance.getEntriesByType(type));
      return results;
    },
    ...
    mark: function(name, type, measure, clear){
      ...
      this.__performance.mark && this.__performance.mark(val);
    ...
    measure: function(name, begin, end){
      ...
      this.__performance.measure && this.__performance.measure(name, vbegin, vend);
    ...
    clearMarks: function(name){
            //name = name.replace(/\./g, "_");
      this.__performance.clearMarks && this.__performance.clearMarks(name);
      ...
      if (name) {
        ...
        this.__performance.clearMarks && this.__performance.clearMarks(vbegin);
        this.__performance.clearMarks && this.__performance.clearMarks(vend);
      }
    ...
    clearMeasure: function(name){
      ...
      this.__performance.clearMeasure && this.__performance.clearMeasure(name);
  ...
  libServices.Class('Logger', com.servicemax.client.lib.api.Object, {
    ...
    getTimestamp: function () {
      return com.servicemax.client.lib.datetimeutils ? com.servicemax.client.lib.datetimeutils.DatetimeUtil.macroDrivenDatetime('Now', 'MM-DD-YYYY', 'HH:mm:ss') : new Date().toString();
    },
